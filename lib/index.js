...
/** 余额查询路由（浏览器侧 fetch 用，不返回 API Key） */
const BALANCE_PATH = '/pet/balance';

/** DeepSeek 官方余额查询端点 */
const DEEPSEEK_BALANCE_URL = 'https://api.deepseek.com/user/balance';

/**
 * 从 $DSH_HOME/.credentials.yaml 读取 DEEPSEEK_API_KEY。
 * 只在本宿主进程内使用，绝不下发到浏览器。
 * @returns API Key；未配置/读取失败时返回 undefined
 */
...
/**
 * 服务端代理查询 DeepSeek 余额。
 * 浏览器只拿到脱敏后的余额 JSON，不接触 API Key。
 * @returns {Promise<object>} { ok, ...balanceFields } 或 { ok:false, error }
 */
async function fetchDeepSeekBalance() {
  const apiKey = readDeepSeekApiKey();
  if (!apiKey) return { ok: false, error: 'missing DEEPSEEK_API_KEY' };
  try {
    ...
    clearTimeout(timer);
    if (!upstream.ok) return { ok: false, error: `upstream ${upstream.status}` };
...
function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // 余额查询：服务端代理 DeepSeek API，浏览器拿不到 Key
      if (url.pathname === BALANCE_PATH) {
        res.writeHead(200, {
          'content-type': 'application/json; charset=utf-8',
          'cache-control': 'no-store',
        });
        res.end(JSON.stringify(await fetchDeepSeekBalance()));
        return;
      }
